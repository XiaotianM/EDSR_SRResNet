...
def SRResNet(t_image, is_train=False, reuse=False):
    ...
    with tf.variable_scope("SRResNet", reuse=reuse):
...
if __name__ == "__main__":
    ...
    net = SRResNet(t_image, is_train=True, reuse=False)
    net_test = SRResNet(t_valid_image, is_train=False, reuse=True)
    ...
    net_vars = tl.layers.get_variables_with_name('SRResNet', True, True)
    ...
    summary_writer = tf.summary.FileWriter("SRResNet_log", sess.graph)
    ...
    for epoch in range(0, n_epoch+1):
        ...
        if (epoch != 0) and (epoch % 10 == 0):
            tl.files.save_npz(net.all_params, name=checkpoint_dir+'/SRResNet.npz')
